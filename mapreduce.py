...
def process_key(key, values):
    minOcuupancy = min(value["passengers"] / value["maxCapacity"] for value in values)
    maxOcuupancy_freq = sum(value["passengers"] == value["maxCapacity"] for value in values)
    print(f"{key}\tmin\t{minOcuupancy}")
    print(f"{key}\tmax\t{maxOcuupancy_freq}")
